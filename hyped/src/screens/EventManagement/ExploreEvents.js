...
export default function ExploreEvents({ searchQuery }) {
    ...
    const getEventList = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                `${env.Market_Place_API_URL}event/get-explore-events`,
                payload,
            ...
            console.error(`Error fetching Explore Events events:`, error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            {loading ? (
                <Text style={styles.loadingText}>{translations.loadingExploreEvents || "Loading explore events..."}</Text>
            ) : events.length === 0 ? (
                <Text style={styles.noEventsText}>{translations.noExploreEvents || "No explore events available"}</Text>
            ) : (
                <FlatList
                    data={events}
