...
const ChatProfile = () => {
    ...
    const chatId = activeChat.chat?.samvada_chinha;
    const username = activeChat.chat?.contact_name ?? activeChat.chat?.samvada_nama;
    const userPhoto = (activeChat.chat?.contact_photo ?? activeChat.chat?.samuha_chitram ?? '') + env.SAS_KEY;
    const otherUserId = activeChat.chat?.pathakah_chinha;
    const hidePhoneNumber = Boolean(activeChat.chat?.hidePhoneNumber);
    const RequeststatusOther = activeChat.chat?.status || "";
    const otherUserPhoneNumber = activeChat.chat?.contact_number ? String(activeChat.chat.contact_number) : null;
